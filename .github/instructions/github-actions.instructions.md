...
The two-pass enumerate + auto-PR drift-back requirements above assume the reconciler is **export-capable** — it exposes `-ExportCurrentState` so tenant state can be round-tripped back into `data-plane/**` YAML as a re-export PR. That assumption does not hold for **Tier-3** surfaces whose underlying cmdlet family has no bulk-export equivalent. The authoritative list of export-incapable reconcilers is the full-circle reconciler-contract-guard exempt list in [`validate.yml`](../workflows/validate.yml).

> **No workflow currently uses this carve-out.** IRM was its reference implementation, and graduated: `Deploy-IRMPolicies.ps1` gained `-ExportCurrentState` and [`deploy-irm.yml`](../workflows/deploy-irm.yml) gained the full two-pass enumerate + drift-back job under [#177](../../issues/177), so both left the exempt list. Every reconciler still on that list (administrative units, classifications, collections, communication compliance, data sources, glossary) has **no** forward workflow at all — backfilling them is tracked upstream in [#80](https://github.com/marcusjacobson/Purview-as-Code/issues/80). The carve-out is kept because whoever writes the first of those workflows needs it; treat it as a specification, not a description of anything shipping today.
>
> Do not read this section as licence to ship an export-incapable workflow when an exporter is *achievable*. IRM's own history is the caution: it was classified Tier-3 on the belief that the cmdlet family had no export path, and an exporter turned out to be perfectly buildable once someone tried. Confirm the limitation against a live probe before claiming it. Separately, an export-incapable surface is not automatically reconcilable at all — see [ADR 0064](../../docs/adr/0064-irm-entity-lists-are-microsoft-managed.md), where the read path for the field carrying operator intent did not exist and the surface was parked rather than given a Tier-3 workflow.

For a Tier-3 forward `deploy-<domain>.yml`, apply the following carve-out.
...
- **No auto-PR drift-back job.** A Tier-3 reconciler cannot re-export, so there is no YAML to regenerate and no drift-back PR to open. Reverse drift is surfaced as a GitHub **issue** by the companion `sync-<domain>-from-tenant.yml`, not a PR. The workflow therefore holds **no** `contents: write` and **no** `pull-requests` scope. ([`sync-irm-from-tenant.yml`](../workflows/sync-irm-from-tenant.yml) is still built this way and remains a working example of the issue-based reverse leg, even though its forward twin is no longer Tier-3.)
