# Runbook: Local DLP drift sync (ADR 0060 local equivalent)

Use this runbook when you need to capture a manual Microsoft Purview portal
edit on the DLP surface into `data-plane/dlp/policies.yaml`, and the
`sync-dlp-from-tenant.yml` scheduled workflow cannot reach the tenant
because of a Key Vault governance lock. See
...
> [!IMPORTANT]
> `scripts/Invoke-LocalDlpDriftSync.ps1` is exclusively for interactive
> runs from the lab owner's workstation. It never runs in CI.

## When to use this runbook

- Lab's daily `sync-dlp-from-tenant.yml` schedule reports its sync job as
  **skipped**, not failed — the ADR 0060 gate (`vars.CI_DATA_PLANE_ENABLED
  = 'false'`) is doing exactly what it is supposed to, and this runbook is
  the intended local fallback.
- You made (or suspect someone made) a manual DLP policy or rule edit in
  the Purview portal and want to confirm whether the repo has drifted,
  without waiting for the next owner-supervised full reconcile.
- You want a read-only tenant audit without touching Key Vault at all
  (`-AuditOnly`), which is also the safe path while
  [issue #160](../../CHANGELOG.md) (`Set-DlpComplianceRule` fails
  app-only) leaves 14 rule-text rows in permanent declared drift — a raw
  `-ExportCurrentState` today would re-quote those texts and undo
  [#157/#158](../../CHANGELOG.md)'s YAML fix.

## When NOT to use this runbook

- You are running CI. `sync-dlp-from-tenant.yml` and `deploy-dlp.yml` are
  the CI paths; this script has no workflow equivalent and is not called
  from any workflow.
- You want to **apply** a repo change to the tenant (create/update/remove
  a policy or rule). That is `scripts/Deploy-DLPPolicies.ps1` directly
...
- Key Vault is reachable from CI for this tenant. Use the scheduled
  workflow or a `workflow_dispatch` run instead; this script exists
  specifically for the governance-locked case.
...
4. PowerShell modules `powershell-yaml` and `ExchangeOnlineManagement`
   installed (same prerequisites as `Deploy-DLPPolicies.ps1`).
...
This runs `Deploy-DLPPolicies.ps1 -DirectionPolicy audit -WhatIf` against
...
JSON audit record to `.copilot-tracking/audit/dlp-<environment>.json`
...
This exports the live tenant's DLP policies into the worktree's copy of
`data-plane/dlp/policies.yaml`, classifies the diff via
`scripts/modules/ExportDiffFilter.psm1` (the same cosmetic-only filter
`sync-dlp-from-tenant.yml` uses), and — only if the diff is meaningful —
commits to `auto/dlp-drift-sync-<environment>`, pushes, and opens or
updates a PR with the same title, labels, and review checklist the CI
workflow uses. Run `-WhatIf` first to preview without committing/pushing.
...
Review it exactly as you would `sync-dlp-from-tenant.yml`'s drift-back
PR: confirm the diff reflects an intended portal edit (not a mistake to
revert in the portal instead), that any `advancedRule` bodies were not
re-serialized, and that SIT/label references still resolve. Merge fires
`deploy-dlp.yml`'s push trigger as normal.

## See also

- [`docs/solutions/compliance/dlp.md`](../solutions/compliance/dlp.md) —
  the general DLP-as-code workflow.
- [`local-cert-provisioning.md`](local-cert-provisioning.md) — cert setup.
- [`dlp-end-to-end-smoke.md`](dlp-end-to-end-smoke.md) — the KV-path smoke
  test this runbook's local-cert path is designed to avoid needing.
