...
<#
.SYNOPSIS
    Local, operator-run equivalent of sync-dlp-from-tenant.yml's sync job,
    for a governance-locked tenant where the scheduled CI run cannot reach
    the tenant (ADR 0060).

.DESCRIPTION
    `sync-dlp-from-tenant.yml`'s scheduled run is a preflight no-op on lab:
    `vars.CI_DATA_PLANE_ENABLED = 'false'` (set 2026-08-16, ADR 0060,
    because lab's Key Vault governance policy blocks the workflow's
    temporary public-network-access step), and `drift-detection.yml`
    carries the same gate. A manual `deploy-dlp.yml workflow_dispatch`
    bypasses the ADR 0060 gate but still fails at Key Vault. The only path
    that reaches lab's tenant today is a local run using the ADR 0028
    local-certificate transport, which never touches Key Vault at all.

    This script reproduces the workflow's sync job locally:
      ...
      3. Runs `Deploy-DLPPolicies.ps1 -ExportCurrentState -Force` (or, under
         `-AuditOnly`, `-DirectionPolicy audit -WhatIf`) against a detached
         git worktree checked out from `<remote>/<BaseBranch>` -- the
         operator's own working-tree checkout is never touched.
      4. Classifies the resulting diff via `Test-ExportDiffMeaningful`
         (scripts/modules/ExportDiffFilter.psm1, shared with the CI
         workflow) and, if meaningful, commits, pushes, and opens or
    ...
    `-DirectionPolicy audit -WhatIf` pass instead of an export -- use this
    while issue #160's 14 rule-text rows remain unresolved, since a raw
    export today would re-quote them and undo #157/#158's YAML fix.
    ...
    References:
      ...
      docs/runbooks/dlp-local-drift-sync.md
...
.PARAMETER SyncBranch
    ...
    auto/dlp-drift-sync-<environment>, matching the CI workflow so the two
    never collide.
...
.EXAMPLE
    ...
    ./scripts/Invoke-LocalDlpDriftSync.ps1 -AuditOnly -WhatIf
...
#        tests/scripts/Invoke-LocalDlpDriftSync.Tests.ps1 -- no az/gh/git
...
function ConvertFrom-DlpInformationCount {
    <#
    .SYNOPSIS
        Parses Deploy-DLPPolicies.ps1's three Write-Information count
        lines ("Desired policies: N", "Tenant policies : N",
        "Tenant rules    : N") out of its captured -InformationVariable.
    ...
    .OUTPUTS
        [pscustomobject] with DesiredPolicies / TenantPolicies /
        TenantRules, each [int] or $null if the line was not present.
        The routine case for a $null DesiredPolicies is not an error at
        all: a SYNC run drives Deploy-DLPPolicies.ps1 with
        -ExportCurrentState, and that script's whole desired-state load
        region is guarded `if ($mode -eq 'Apply')`, so
    ...
    foreach ($line in $Lines) {
        ...
        } elseif ($line -match '^\s*Tenant rules\s*:\s*(\d+)\s*$') {
            $result.TenantRules = [int]$Matches[1]
        }
    }
    return $result
}
...
function Get-DesiredPolicyCountFromYaml {
    ...
    .DESCRIPTION
        ...
        export, and Deploy-DLPPolicies.ps1 never emits the
        "Desired policies: N" line in that mode -- its desired-state load
        region is guarded `if ($mode -eq 'Apply')`, because an export
        overwrites $Path and must not fail on a file that does not parse.
...
function ConvertTo-DlpAuditRecord {
    <#
    .SYNOPSIS
        Builds the JSON-serializable audit record written to
        .copilot-tracking/audit/dlp-<environment>.json.
    ...
    .PARAMETER Rows
        Category/Kind/Name/Reason rows from Deploy-DLPPolicies.ps1's
        pipeline output (audit mode only; empty in export mode).
    #>
    [CmdletBinding()]
    [OutputType([pscustomobject])]
    param(
    ...
    return [pscustomobject]@{
        ...
        counts        = [pscustomobject]@{
            ...
            tenantRules     = $Counts.TenantRules
        }
        ...
        rows          = @($Rows)
        pr            = $pr
        tool          = 'Invoke-LocalDlpDriftSync.ps1'
    }
}
...
# --- Parameters file resolution (mirrors Deploy-DLPPolicies.ps1) ---
...
if ([string]::IsNullOrWhiteSpace($env:PURVIEW_LOCAL_CERT_THUMBPRINT)) {
    throw "`$env:PURVIEW_LOCAL_CERT_THUMBPRINT is not set. This script requires the ADR 0028 local-certificate transport -- see docs/runbooks/dlp-local-drift-sync.md."
}
...
if (-not $SyncBranch) { $SyncBranch = "auto/dlp-drift-sync-$environmentName" }
...
$worktreePath = Join-Path ([System.IO.Path]::GetTempPath()) ("dlp-drift-sync-{0}-{1}" -f $environmentName, [guid]::NewGuid().ToString('N').Substring(0, 8))
...
try {
    ...
    $dlpScriptPath = Join-Path $worktreePath 'scripts/Deploy-DLPPolicies.ps1'
    $yamlPath = Join-Path $worktreePath 'data-plane/dlp/policies.yaml'
    ...
    if ($AuditOnly.IsPresent) {
        $rows = & $dlpScriptPath -DirectionPolicy audit -WhatIf -Confirm:$false -Path $yamlPath -ParametersFile $ParametersFile -InformationVariable iv 6>$null
    } else {
        & $dlpScriptPath -ExportCurrentState -Force -Confirm:$false -Path $yamlPath -ParametersFile $ParametersFile -InformationVariable iv 6>$null | Out-Null
    }
    $infoLines = @($iv) | ForEach-Object { [string]$_.MessageData }
    $counts = ConvertFrom-DlpInformationCount -Lines $infoLines
    if ($null -eq $counts.DesiredPolicies) { $counts.DesiredPolicies = $desiredCountBeforeExport }
    Write-Information ("Desired policies: {0}  Tenant policies: {1}  Tenant rules: {2}" -f $counts.DesiredPolicies, $counts.TenantPolicies, $counts.TenantRules) -InformationAction Continue
    ...
    if (-not $AuditOnly.IsPresent) {
        $diffText = (Invoke-ChildProcess -FilePath 'git' -ArgumentList @('-C', $worktreePath, 'diff', '-U0', '--', 'data-plane/dlp/policies.yaml')) -join "`n"
        ...
        if (-not $meaningful) {
            ...
            if (-not [string]::IsNullOrEmpty($diffText)) {
                Invoke-ChildProcess -FilePath 'git' -ArgumentList @('-C', $worktreePath, 'checkout', 'HEAD', '--', 'data-plane/dlp/policies.yaml') | Out-Null
            }
        } else {
            $diffDetected = $true
            ...
            if ($PSCmdlet.ShouldProcess($SyncBranch, "commit, push, and open/update the drift-back PR against $BaseBranch")) {
                ...
                Invoke-ChildProcess -FilePath 'git' -ArgumentList @('-C', $worktreePath, 'add', 'data-plane/dlp/policies.yaml') | Out-Null
                $commitMessage = "chore(data-plane): sync DLP policies from tenant`n`nAutomated drift-back from scripts/Invoke-LocalDlpDriftSync.ps1 (ADR 0060).`nRe-exports the live Microsoft Purview Data Loss Prevention (DLP) policy`nset (and rules) in the $environmentName tenant into`ndata-plane/dlp/policies.yaml so the repo remains source of truth."
                ...
                $bodyText = "Automated drift-back from ``scripts/Invoke-LocalDlpDriftSync.ps1`` (ADR 0060 local equivalent of ``.github/workflows/sync-dlp-from-tenant.yml``, run manually because lab's scheduled sync is a preflight no-op under the same ADR).`n`nThe live Microsoft Purview Data Loss Prevention (DLP) policy set (and rules) in the $environmentName tenant no longer matches ``data-plane/dlp/policies.yaml``. This PR re-exports the tenant state via ``scripts/Deploy-DLPPolicies.ps1 -ExportCurrentState`` so the repo regains source-of-truth before the next ``deploy-dlp.yml`` apply.`n`n## Review checklist`n`n- [ ] The diff reflects an intended portal edit. If the portal edit was a mistake, **close this PR** and revert the edit in the Microsoft Purview portal instead.`n- [ ] ``advancedRule`` bodies in the exported diff are the intended ADR 0031 shape and were not re-serialized (byte-identity is a tracked-field guarantee).`n- [ ] SIT (sensitive-information-type) GUID references and workload-location buckets resolve to real tenant objects and are not stale.`n- [ ] After merge, the next ``deploy-dlp.yml`` push run should report 0 drift under ``direction_policy=portal-wins``.`n`n## Notes`n`n- Trigger: local operator run via ``scripts/Invoke-LocalDlpDriftSync.ps1``, base commit ``$baseCommit``."
                ...
                try {
                    ...
                    if (-not $existing -or @($existing).Count -eq 0) {
                        $created = Invoke-GhApiCall -Path "$repoPath/pulls" -Method POST `
                            -Field @('title=chore(data-plane): sync DLP policies from tenant', "head=$SyncBranch", "base=$BaseBranch") `
    ...
    if (-not $NoAuditRecord.IsPresent) {
        ...
        if (-not $recordPath) {
            ...
            $recordPath = Join-Path $recordRoot "dlp-$environmentName.json"
        }
        $record = ConvertTo-DlpAuditRecord `
